...
所有模块都先通读本{{TURN_UNIT}}奏章三大模块，建立“本{{TURN_UNIT}}到底发生了什么”的时间线

#### 1.1.1 诏书核销

读本{{TURN_UNIT}}诏书逐条落地情况：力度（投了多少银/人/权限）、阻力、结果三态。**核销章每条标题括号写的是 `（结果：完成/进行中/中断）`，不是 `（进展：档位）`**——这是诏书核销专用的结果词，照它判该条去向：
